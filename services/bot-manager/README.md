...
**Bot capability flags** (passed in `POST /bots` body, forwarded to bot container via `BOT_CONFIG`):

| Flag | Default | Effect |
|------|---------|--------|
| `transcribe_enabled` | `true` | Per-speaker transcription pipeline |
| `recording_enabled` | `true` | Audio file recording + upload to storage |
| `tts_enabled` | `false` | PulseAudio gain=0 idle, gain=1 during /speak. No camera. |
| `video_receive_enabled` | `false` | Receive+decode video from participants |
| `camera_enabled` | `false` | Outgoing virtual camera/avatar |
| `voice_agent_enabled` | `false` | **DEPRECATED**: alias for `tts_enabled: true`. No camera. |

Speaker metadata is always recorded (never gated behind any flag). Each flag is independent -- `tts_enabled` does NOT enable camera, `video_receive_enabled` does NOT enable mic. Default bot = transcribe + record + speaker metadata.
