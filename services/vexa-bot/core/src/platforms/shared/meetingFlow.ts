...
export async function runMeetingFlow(
  ...
  try {
    ...
    try {
      ...
      log(`[MeetingFlow] Recording/monitoring error: ${error?.name}: ${error?.message}`);
      if (error?.stack) {
        log(`[MeetingFlow] Stack: ${String(error.stack).split('\n').slice(0, 5).join(' | ')}`);
      }
      const errorDetails = {
        error_message: error?.message,
        error_stack: error?.stack,
    ...
    log(`[MeetingFlow] Outer catch - error: ${error?.name}: ${error?.message}`);
    if (error?.stack) {
      log(`[MeetingFlow] Stack: ${String(error.stack).split('\n').slice(0, 5).join(' | ')}`);
    }
    const errorDetails = {
