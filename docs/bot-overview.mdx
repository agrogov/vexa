...
Bot capabilities are independent flags. Each does one thing:

| Capability | Flag | Default | Description |
|-----------|------|---------|-------------|
| Transcribe | `transcribe_enabled` | `true` | Audio capture, per-speaker pipeline, save to DB |
| Record audio | `recording_enabled` | `true` | MediaRecorder, save audio file |
| Speaker metadata | (always on) | ALWAYS | Speaker events, timestamps -- needed for post-meeting pipeline |
| TTS (speak) | `tts_enabled` | `false` | PulseAudio gain=0 idle, gain=1 during /speak only |
| Incoming video | `video_receive_enabled` | `false` | Receive+decode video from participants |
| Camera | `camera_enabled` | `false` | Outgoing virtual camera/avatar |

Default bot = transcribe + record + speaker metadata. Everything else OFF.

`voice_agent_enabled` is **deprecated** -- treat as alias for `tts_enabled: true`. No camera.
